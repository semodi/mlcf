...
#    h2o = read('start.traj')
    ...
    scaler_o_slow = pickle.load(open(model_path + 'scaler_O_mol','rb'))
    scaler_h_slow = pickle.load(open(model_path + 'scaler_H_mol','rb'))
    descr_getter_slow.set_scalers(scaler_o_slow, scaler_h_slow)
    descr_getter_slow.single_thread = single_thread_descriptors_molecular
    calc_slow.set_feature_getter(descr_getter_slow)

    krr_o_slow = keras.models.load_model(model_path + 'force_O_mol')
    krr_h_slow = keras.models.load_model(model_path + 'force_H_mol')
