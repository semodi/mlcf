...
class Force_Network():
    ...
    def train(self,
        ...
        if not self.compiled:
            ...
            raise Exception('Step size and regularization can not be changed\
                            after training was started, please re-compile model')
    ...
    def evaluate(self, plot = False):
        prediction = self.predict(self.X_test, True)
        error = self.y_test - prediction
        ...
        if plot:
            plt.figure()
            plt.plot(self.y_test, prediction, ls = '', marker = '.')
            plt.xlabel('Expected')
    ...
    def save_all(self, net_dir, override = False):
        ...
        if os.path.exists(net_dir + 'force_' + self.species) and not override:
            ...
            pickle.dumps(open(net_dir + 'supp_' + self.species, 'wb'))
            self.model.save(net_dir + 'force_' + self.species)

    def load_all(self, net_dir):
        supp = pickle.loads(open(net_dir + 'supp_' + self.species, 'rb'))
        self.model = kerals.models.load_model(net_dir + 'force_' + self.species)
        self.mask = supp['mask']
...
def load_force_model(net_dir, species):
    model = Force_Network(species, None, None)
